...
bool parseGoalFile(std::string filename){
  ...
  while(!feof(file) && strlen(sTemp) != 0){
    if(strcmp(sTemp, "pickup_table") == 0){
      if(fscanf(file,"%f %f %f",&v[0],&v[1],&v[2]) < 1)
        ROS_ERROR("Error parsing pickup_table goal.");
      pickup_table_goal = v;
    }
    else if(strcmp(sTemp, "putdown_table") == 0){
      if(fscanf(file,"%f %f %f",&v[0],&v[1],&v[2]) < 1)
        ROS_ERROR("Error parsing putdown_table goal.");
      putdown_table_goal = v;
    }
    else
      ROS_ERROR("Can't parse: %s", sTemp);
    if(fscanf(file,"%s",sTemp) < 1)
...
vector<float> pickup_table_goal;
vector<float> putdown_table_goal;
Arm* larm;
Arm* rarm;
...
int main(int argc, char** argv){
  ...
  //go to table
  ROS_INFO("Go to pickup table");
  demo.goToPose(demo.pickup_table_goal);

  //pick up basket
  ROS_INFO("Pick up basket");
  demo.pickup();

  //carry basket to other table
  ROS_INFO("Go to put down table");
  demo.goToPose(demo.putdown_table_goal);

  //put down basket
  ROS_INFO("Put down basket");
  demo.putdown();
